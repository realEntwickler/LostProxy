...
public class IParty {
    ...
    public void addMember (ProxiedPlayer proxiedPlayer) {
        ...
        sendMessage(new MessageBuilder(Prefix.PARTY + new IPlayer(proxiedPlayer.getUniqueId()).getDisplaywithPlayername() + " §7ist der Party beigetreten§8.").build());
    }

    public void removeMember (ProxiedPlayer proxiedPlayer) {
        members.remove(proxiedPlayer);
        sendMessage(new MessageBuilder(Prefix.PARTY + new IPlayer(proxiedPlayer.getUniqueId()).getDisplaywithPlayername() + " §7hat die Party verlassen§8.").build());
    }
